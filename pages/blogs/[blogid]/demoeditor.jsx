        ...
        input.onchange = (e) => {
            ...
            reader.onload = (ev) => {
                ...
                img.addEventListener("dragend", () => {
                    img.classList.remove(styles.dragging);
                });

                // Allow dropping images between blocks
                editorRef.current.addEventListener("dragover", (e) => e.preventDefault());
                editorRef.current.addEventListener("drop", (e) => {
                    e.preventDefault();
                    const dragging = document.querySelector(`.${styles.dragging}`);
                    if (dragging) {
                        const range = document.caretRangeFromPoint(e.clientX, e.clientY);
                        range.insertNode(dragging.parentElement);
                        updateContent();
                    }
                });
